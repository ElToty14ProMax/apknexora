...
import { compactId, copyText, dateTime, fileToBase64, isValidCpf, money, onlyDigits, sha256Hex } from "./utils";
...
function AuthPanel({
  ...
  const submitRegister = async (event: FormEvent) => {
    ...
    setBusy(true);
    try {
      await api.register({ ...register, cpf: cpfDigits });
      ...
      <div className="auth-card">
        ...
        {mode === "register" && (
          <form onSubmit={submitRegister} className="form-grid">
            ...
            <label>
              Data de nascimento
              <input type="date" value={register.birthdate} onChange={(event) => setRegister({ ...register, birthdate: event.target.value })} required max={new Date().toISOString().split("T")[0]} />
            </label>
            <label>
              Chave Pix
              <input value={register.pixKey} onChange={(event) => setRegister({ ...register, pixKey: event.target.value })} required />
            </label>
            <label>
              Senha
              <input
...
function CommunityView({
  ...
  const contribute = async (requestId: string) => {
    const amountCents = Math.round(Number(amounts[requestId] || 0) * 100);
    setBusyAction(`contribute-${requestId}`);
    try {
      const result = await api.createContribution(token, requestId, amountCents);
      onInstruction(result);
      await reload();
    } catch (error) {
      showNotice(error instanceof Error ? error.message : "Não foi possível apoiar.", "error");
    } finally {
      setBusyAction(null);
    }
  };
      ...
      <div className="request-grid">
        {community.map((item) => (
          <article className="support-card" key={item.id}>
            ...
            <div className="money-line">{money(item.amountCents - item.fundedCents)} disponível</div>
            <p>Recebedor {item.requesterPublicId} - Nivel {item.requesterLevel}</p>
            <div className="progress"><span style={{ width: `${Math.min(100, (item.fundedCents / item.amountCents) * 100)}%` }} /></div>
          </article>
        ))}
      </div>
...
function ReceiptCard({
  ...
  const scanReceipt = async () => {
    if (!file) {
      showNotice("Selecione a foto do comprovante primeiro.", "error");
      return;
    }
    setOcrBusy(true);
    setOcrResult(null);
    try {
      ...
      if (!result.transactionId && !result.amountCents) {
        showNotice("Não foi possível identificar dados no comprovante.", "error");
      }
    } catch (error) {
      showNotice(error instanceof Error ? error.message : "Análise falhou.", "error");
    } finally {
      setOcrBusy(false);
    }
  };
    ...
    <article className="history-card">
      ...
      {!alreadySent && item.status === "PENDING_ADMIN" && (
        <div className="receipt-form">
          <input value={transactionId} onChange={(event) => setTransactionId(event.target.value)} placeholder="ID da transação Pix" required />
          <input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { setFile(event.target.files?.[0] || null); setOcrResult(null); }} required />
          <div className="receipt-ocr-actions">
              <button className="btn-scan" onClick={scanReceipt} disabled={ocrBusy || !file}>
              {ocrBusy ? <Loader2 className="spin" size={18} /> : <Scan size={18} />}
              {ocrBusy ? "Analisando..." : "Analisar com IA"}
            </button>
...
function ProfileView({ profile, showNotice }: { profile: Profile; showNotice: (text: string, kind?: "ok" | "error") => void }) {
  ...
  const shareText = `Entra comigo na Nexora usando meu convite ${profile.inviteCode}: ${inviteUrl}`;
  const shareWhatsapp = () => {
    ...
    <section className="two-columns">
      <Panel title="Perfil">
        <div className="profile-block">
          ...
          <CopyField label="Pix cadastrado" value={profile.pixKeyMasked} showNotice={showNotice} />
        </div>
      </Panel>
      ...
      <Panel title="Taxa administrativa">
        ...
        {profile.adminPixKey ? (
          <>
            ...
            <CopyField label="Pix do admin" value={profile.adminPixKey} showNotice={showNotice} />
          </>
        ) : (
            ...
            <table className="contributions-table">
              ...
              <tbody>
                {filteredContributions.map((item) => (
                  <tr key={item.id}>
                    <td><strong>{item.requestPublicCode}</strong><span>{statusLabel(item.status)}</span><small>{compactId(item.id)}</small></td>
      ...
      {selectedUser && (
        <DetailModal title={selectedUser.name} subtitle="Dados completos do usuário" onClose={() => setSelectedUser(null)}>
          <DetailGrid items={[
            ...
            ["Pix do admin", selectedUser.adminPixKey || "-"],
      ...
      {selectedRequest && (
        <DetailModal title={selectedRequest.publicCode} subtitle="Detalhes da solicitação" onClose={() => setSelectedRequest(null)}>
          <DetailGrid items={[
            ...
            ["Chave Pix", selectedRequest.requesterPixKey],
  ...
  return (
    <div className="modal-backdrop" role="dialog" aria-modal="true">
      <div className="modal">
        ...
        <p className="secure-note">A chave Pix da pessoa que recebe não é exibida. Depois da transferência, anexe o ID e a foto no histórico.</p>
      </div>
    </div>
  );
}
          ...
          : label.includes("Validar retorno")
            ...
            : label.includes("Validar Pix")
              ...
              : label.includes("Bloquear") || label.includes("Recusar")
                ...
                : <Check size={16} />;
  return (
    <button className={cn("small-action", danger && "danger")} onClick={onClick} disabled={busy || disabled} title={label} aria-label={label}>
