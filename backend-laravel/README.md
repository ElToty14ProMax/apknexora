...
Pix instructions return the requester's registered random Pix key, the EVP key generated by that person's bank and stored encrypted in Nexora. The app copies that key for the donor and does not generate a BR Code that embeds the requester's name. Receipt dates are assigned by the server when evidence is submitted.
